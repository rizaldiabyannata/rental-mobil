...
// Function to find the price for '2-3 PAX' among all tiers for a package
const getStartingPrice = (tourPackage) => {
  if (!tourPackage.hotelTiers || tourPackage.hotelTiers.length === 0) {
    return 0;
  }
  // Cari priceTier dengan paxRange persis '2-3 PAX'
  for (const tier of tourPackage.hotelTiers) {
    const found = tier.priceTiers?.find(
      (pt) => (pt.paxRange || "").toLowerCase() === "2-3 pax"
    );
    if (found) return found.price;
  }
  // Jika tidak ada, fallback ke harga terendah
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/admin/tour-packages/${packageToDelete.id}`,
        {
          method: "DELETE",
        }
      );
                  ...
                  {!loading &&
                    ...
                    packages.map((pkg) => (
                      <TableRow key={pkg.id}>
                        ...
                        <TableCell>
                          {formatCurrency(getStartingPrice(pkg))}
                        </TableCell>
                        <TableCell className="text-right">
                          <div className="flex justify-end gap-2">
                            <Button variant="ghost" size="sm" asChild>
                              <Link
                                href={`/admin/tour-packages/edit/${pkg.id}`}
                              >
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Tindakan ini akan menghapus paket tour "{packageToDelete?.name}".
